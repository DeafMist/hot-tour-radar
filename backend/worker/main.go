...
func processMessage(ctx context.Context, log *slog.Logger, esClient newsIndexer, cache *dedupe.Cache, cfg *config.Worker, msg kafka.Message) error {
	...
	text := processing.CleanText(payload.Text)
	if title == "" && text == "" {
	...
	keywords := processing.ExtractKeywords(title+" "+text, cfg.KeywordLimit, cfg.KeywordMinLength)
	...
	doc := models.NewsDocument{
		ID:        processing.BuildDocumentID(title, text, ts),
		Title:     title,
		Text:      text,
		Timestamp: ts,
		Keywords:  keywords,
